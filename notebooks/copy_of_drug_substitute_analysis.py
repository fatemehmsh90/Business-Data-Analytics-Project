# -*- coding: utf-8 -*-
"""Copy of Drug_Substitute_Analysis.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1KQvk_NACqRNNRx9w1FKXmpiHZtto9c2a
...
"""In the implementation, I first created one field called combined_text for each drug. This field joins the three main description parts: **ProductIntroduction**, **SideEffect**, and **HowWorks** into one long text. For example, for “Andol 0.5mg Tablet” and “Actorise 40 Injection” the combined text already contains both the basic description and information about side effects and how the drug works.

After this step, I checked the length of the combined texts. The dataset has **192,807** rows. The average length of the combined text is about **1,839** characters, with a minimum of **6** and a maximum of about **4,166** characters. Half of the drugs have a length between roughly **1,600** and **2,165** characters. This means that most drugs have enough textual information for the embedding model, and the variation in length is still acceptable for the later similarity calculations.
...
"""After creating the combined text for each drug, the next step was generating text embeddings. Because the full MID dataset is very large, I used a big random sample of 30,000 records for this step. This sample still covers many therapeutic and chemical classes, so it keeps good variety in the data but reduces the computation time.

I used the light Sentence-BERT model all-MiniLM-L6-v2. The model was loaded on CPU because no GPU was available in the environment. Even with the smaller model, the embedding step took a long time, a bit more than one hour, which shows how computationally heavy this process is.

The embedding process finished successfully. The final output was a matrix with
30,000 rows and 384 columns, meaning each drug was converted into a 384-dimensional vector. This vector is normalized and can be directly used for computing cosine similarity in the next steps of the project. The result also shows that the model worked in stable way and handled the large batch of text without errors.
...
"""To use the non-text fields of the MID dataset, four structured columns were selected: Chemical_Class, Habit_Forming, Therapeutic_Class, and Action_Class. These fields include many repeated labels and many spelling or spacing variations, so they were cleaned first. All missing values were replaced with “Unknown”, and extra spaces were removed.

After cleaning, the four columns were transformed into numerical vectors using One-Hot Encoding. This method creates one binary feature for every unique category in the dataset. Because the dataset is large and contains many drug classes, the encoded matrix became:

- **192,807 rows**

-	**1,324 encoded features**

This size shows that the chemical and therapeutic classifications are very diverse. Keeping all categories (without dropping any level) was important, because we did not want to lose rare but meaningful drug information.

The encoded feature names also confirm this variety. For example, the first labels include:

- **Chemical_Class_1,2-Aminoalcohols**
- **Chemical_Class_1-Benzopyrans derivative**
- **Chemical_Class_21-hydroxysteroids**

This encoding is later combined with the text embeddings to create a complete feature vector for each drug.
...
"""After creating the text embeddings and the one-hot encoded structured features, the next step was to combine them into one feature matrix. The text embeddings had a shape of (30000, 384), meaning each drug sample was represented by a 384-dimensional vector. The structured data had a shape of (30000, 1324) because the one-hot encoder expanded the categorical fields into 1324 binary columns.

To merge both parts, the two matrices were placed next to each other using a horizontal concatenation. This produced a final combined feature matrix with shape (30000, 1708). In this way, each drug was described with both types of information at the same time: its text description (How it works, Side effects, Introduction) and its structural categories. This combined vector will be used in the next steps for computing similarity, clustering, and building the substitution network. The goal of this integration is to make sure the model does not rely on only one source of information, but sees a more complete picture of each drug.
...
### **6.3.7	Network Construction**

## **6.3	PARAMETER TUNING**

In this project, the parameter tuning is done in a simple and controlled way, because the goal is not to build a very complex model, but a model that is repeatable and easy to explain for drug similarity detection.

In the text embedding part, because a light Sentence-BERT model is used, the main parameters of the model are not changed from their default values. This helps to keep the speed high and makes it possible to run the model on the large MID dataset. The only action taken is setting the embedding output size and using the normalization mode, so we can calculate cosine similarity in a more accurate way.

In the structured features part, no complex parameter tuning is needed. The only important step is using One-Hot Encoding without dropping categories, to make sure small structural details are not removed and are included in the combined model.

In the similarity calculation and clustering stage, two important parameters are adjusted:

1. choosing a threshold to keep only the important links in the similarity network. This value is selected by checking the distribution of similarities, so the network does not become too crowded.

2. choosing the number of clusters in methods such as K-Means or Agglomerative. This value is selected by looking at the pattern of similarities and the goal of the project, and not by heavy parameter search, because the goal here is not prediction accuracy, but to find a logical structure between drugs.

Overall, the parameter tuning in this project is kept simple, so the model stays easy to understand, easy to repeat, and suitable for real-world use, while also respecting computational limits. Since the model is light and the pipeline uses batch processing, performance remained stable on Google Colab and no additional optimization techniques were required.

## **6.4	GITHUB DOCUMENTATION**
...
4.	Aronson, J.K., Ferner, R.E., & Heneghan, C. (2023a). Drug shortages. Part 1: Definitions and harms. British Journal of Clinical Pharmacology, 89(10), pp. 2950–2956.
[Online] Available at: https://doi.org/10.1111/bcp.15842 [Accepted 20 Jun 2023].

5.	Aronson, J.K., Ferner, R.E., & Heneghan, C. (2023b). Drug shortages. Part 2: Trends, causes and solutions. British Journal of Clinical Pharmacology, 89(10), pp. 2957–2963.
[Online] Available at: https://doi.org/10.1111/bcp.15853 [Accepted 20 Jun 2023].

6.	Berral-González, A., Arroyo, M.M., Alonso-López, D., Rivas-López, M.J., Sánchez-Santos, J.M., De Las Rivas. J.D, (2025). Pharmacogenomic drug–target network analysis reveals similarity profiles among FDA-approved cancer drugs. Pharmaceutics, 17(11), 1421, [Online] Available at: https://doi.org/10.3390/pharmaceutics17111421 [Accessed 25 Oct 2025].

7.	Iyer, S.S. (2025). Data-Driven Decision Making: The Key to Future Health Care Business Success. RA Journal of Applied Research, 11(3), pp. 115–136. [Online] Available at: https://doi.org/10.47191/rajar/v11i3.06 [Accepted 03 Mar 2025].

8.	Jeon, W., Park, M., An, D., Nam, W., Shin, J.Y., Lee, S. & Lee, S. (2025). Predicting Drug–Side Effect Relationships from Parametric Knowledge Embedded in Biomedical BERT Models: Methodological Study with an NLP Approach. JMIR Medical Informatics, 13(1), e67513. [Online] Available at: https://medinform.jmir.org/2025/1/e67513 (doi:10.2196/67513) [Accepted 10 Jul 2025].

9.	Kauffman, J., Miotto, R., Klang, E., Costa, A., Norgeot, B., Zitnik, M., Khader, S., Wang, F., Nadkarni, G.N., & Glicksberg, B.S. (2025). Embedding Methods for Electronic Health Record Research. Annual Review of Biomedical Data Science, 8, pp. 563–590. [Online] Available at: https://doi.org/10.1146/annurev-biodatasci-103123-094729 [Accepted 01 May 2025].

10.	Szymańska, A., Król, M. & Baczyński, K. (2025). Comparative Analysis of Molecular Embeddings for Efficient Compound Similarity Search Using Vector Databases. ChemRxiv, 15(1), [Online] Available at: https://doi.org/10.26434/chemrxiv-2025-zvgwq [Accepted 15 Apr 2025].

11.	Yamagiwa, H., Hashimoto, R., Arakane, K., Murakami, K., Soeda, S., Oyama, M., Zhu, Y., Okada, M. & Shimodaira, H. (2025). Predicting Drug–Gene Relations via Analogy Tasks with Word Embeddings. Scientific Reports, 15(17240), [Online] Available at: https://doi.org/10.1038/s41598-025-01418-z [Accessed: 18 May 2025].
"""